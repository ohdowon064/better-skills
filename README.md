...
### `/manage-skills` — 스킬 유지보수 & SSOT 관리

코드베이스 변화에 맞춰 verify 스킬의 드리프트를 탐지하고 수정합니다. Registered Verify Skills 테이블이 전체 시스템의 Single Source of Truth(SSOT)로 기능하며, verify-implementation과 CLAUDE.md는 이 테이블을 런타임에 참조합니다.
...
│   ├── manage-skills/SKILL.md          # 스킬 유지보수 & SSOT
