...
- **리뷰 모드**: `phase`, `integration`, 또는 `standalone`
- **phase 모드**: Phase 번호, PLAN 문서 경로, Phase 시작 git ref
- **integration 모드**: PLAN 문서 경로, 전체 기능 시작 git ref
- **standalone 모드**: diff 범위만 제공 (PLAN 없음)

PLAN 경로가 전달되지 않으면 자동으로 `standalone` 모드로 동작한다.
...
PLAN 없이 일반 코드 품질 기준으로 리뷰한다. `/code-review`에서 PLAN이 없을 때 사용된다.

### 1. Diff 수집

```bash
# code-review 스킬이 전달한 diff 범위 사용
git diff <provided-range>
```

### 2. 코드 리뷰

PLAN 충실도 평가를 제외한 나머지 항목을 동일하게 평가한다:

| 리뷰 항목 | 평가 기준 |
|-----------|----------|
| 설계 품질 | 단일 책임, 의존성 방향, 추상화 수준 |
| 네이밍 | 변수/함수/클래스명의 명확성, 일관성 |
| 에러 핸들링 | 예외 처리 누락, 에러 삼킴, 부적절한 catch |
| 보안 패턴 | SQL injection, XSS, 하드코딩된 시크릿 |
| 성능 패턴 | N+1 쿼리, 불필요한 루프, 메모리 누수 |
| 기존 코드 일관성 | 기존 코드의 패턴/컨벤션을 따르는지 |

### 3. 기존 코드 패턴 파악

Phase 모드와 동일하게 같은 디렉토리의 기존 파일 1-2개를 샘플로 읽어 일관성을 평가한다.
