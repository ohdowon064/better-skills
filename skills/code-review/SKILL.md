...
- PLAN이 있고 특정 Phase에 매핑되면 `phase` 모드로 실행한다
- PLAN이 있지만 특정 Phase에 매핑되지 않거나, `--branch`로 넓은 범위를 리뷰하면 `integration` 모드로 실행한다
- PLAN이 없으면 PLAN 컨텍스트 없이 리뷰한다 (code-reviewer에 PLAN 경로를 전달하지 않음)
- 전달 항목: diff 범위, PLAN 경로 (있으면), git ref (있으면)
