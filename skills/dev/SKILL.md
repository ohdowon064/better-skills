...
새 PLAN이 생성된 후, `feature-planner`를 LIST 모드로 실행하여 진행 중인 PLAN 간 파일 충돌을 확인한다.

feature-planner LIST 모드가 충돌 감지 결과를 반환한다 (충돌 감지 로직은 feature-planner가 SSOT로 관리).

**결과 분기:**

- **충돌 없음** → 메시지 없이 Step 3으로 바로 진행
- **충돌 있음** → AskUserQuestion으로 확인:
...
**⛔ 각 Phase는 반드시 3a → 3b → 3c → 3d → 3e 전체를 순서대로 실행해야 완료된다. 어떤 단계도 건너뛰거나 생략할 수 없다.**

```
각 Phase에 대해:
    retry_count = 0  ← Phase 진입 시 초기화
    3a. Phase 상태를 🔄 In Progress로 변경
    3b. TDD 개발 실행
    3c. 검증 실행 ← TDD 끝나면 반드시 여기로
    3d. 코드 리뷰 ← 검증 끝나면 반드시 여기로
    3e. Phase 완료 처리 (Status → ✅ Complete, retry_count = 0, 다음 Phase)
        검증 실패 시 → retry_count += 1, 수정 후 3c 재실행
        리뷰 REQUEST_CHANGES → retry_count += 1, 수정 후 3c 재실행
        retry_count >= 3 → 사용자에게 블로커 보고, 계속/중단 선택
```
...
**⛔ TDD 개발이 끝나도 Phase가 완료된 것이 아니다. 반드시 Step 3c(검증) → Step 3d(코드 리뷰) → Step 3e(Phase 완료)를 순서대로 실행해야 한다. TDD 완료 후 멈추지 마라.**
...
**⛔ 검증 완료 후 반드시 Step 3d(코드 리뷰)를 실행한다. 검증 통과했다고 Phase를 완료 처리하지 마라.**
...
**⛔ 코드 리뷰 완료 후 반드시 Step 3e(Phase 완료 처리)로 진행한다.**
...
**⛔ 모든 Phase가 ✅ Complete이면 여기서 멈추지 말고 반드시 Step 3.5(통합 코드 리뷰) → Step 4(완료 처리) → Step 5(스킬 유지보수) → Step 6(최종 보고)까지 실행한다.**
